/*
 * pthreads program with a race condition (used int he gdb pthreads example)
 * to compile:
...
// the thread main routine much match this prototype
...
int main(int argc, char** argv)
{
    ...
    ntids = atoi(argv[1]);
    // ensure a value number for ntids
    ...
    {
        tid_args[i] = i;
        ret = pthread_create(&tids[i], 0, worker_loop, &tid_args[1]);
        if (ret)
        {
...
void* worker_loop(void* arg)
{
    ...
    printf("hello I'm thread %d with pthead_id %lu\n",
           myid,
           pthread_self()
    );
